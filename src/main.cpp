...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        ...
        std::cerr << "Modes:\n";
        std::cerr << "  -c : Pure Compression (Encode Only)\n";
        std::cerr << "  -d : Pure Decompression (Decode Only)\n";
        return 1;
    }
    ...
    if (mode_flag == "-c") {
        ...
        std::cerr << "Error: Unrecognized flag '" << mode_flag << "'. Use -c or -d.\n";
        return 1;
    }

    // Dynamic component settings configuration loader
    ...
    std::cout << "\n======================================================\n";
    std::cout << " BZIP2 DZIP ENGINE (STRICT OPERATIONS CONTEXT)\n";
    std::cout << "======================================================\n";
    std::cout << "Active Operational Profile: " << (CONFIG_RUN_ENCODE ? "[PURE ENCODER MODE]" : "[PURE DECOMPRESSOR MODE]") << "\n\n";

    // ------------------------------------------------------------------------
    // MODE 1: PURE DECOMPRESSOR ENGINE (-d)
    // ------------------------------------------------------------------------
    if (!CONFIG_RUN_ENCODE) {
        ...
        if (!compressed_stream) {
            std::cerr << "Error: Could not open compressed container file " << input_file << "\n";
            return 1;
        }
        ...
        std::cout << ">>> STARTING PURE DECODING PIPELINE <<<\n\n";
        ...
        std::cout << "======================================================\n";
        std::cout << "            DECOMPRESSION METRICS PROFILE SUMMARY     \n";
        std::cout << "======================================================\n";
        std::cout << "  Restored File Clone Size: " << dec_len << " bytes\n";
        std::cout << "  Total Execution Latency:  " << global_runtime << " ms\n";
        std::cout << "------------------------------------------------------\n";
    ...
    for (int i = 0; i < manager.num_blocks; ++i) {
        ...
        // Pack structural tracking layout markers to binary stream
    ...
    // Encoder-Only Summary Dashboard
    ...
    std::cout << "\n======================================================\n";
    std::cout << "            COMPRESSION METRICS ENGINE LOGS           \n";
    std::cout << "======================================================\n";
    ...
    std::cout << "  Total Encode Latency: " << global_runtime << " ms\n";
    std::cout << "------------------------------------------------------\n";
    std::cout << "            STAGE-BY-STAGE LATENCY PROFILER           \n";
    std::cout << "------------------------------------------------------\n";
    std::cout << "  [Stage Name]          [Encode Latency]\n";
    std::cout << "  1. RLE-1 Module:       " << std::setw(10) << total_rle1_time << " ms\n";
    std::cout << "  2. BWT Module:         " << std::setw(10) << total_bwt_time << " ms\n";
    std::cout << "  3. MTF Module:         " << std::setw(10) << total_mtf_time << " ms\n";
    std::cout << "  4. RLE-2 Module:       " << std::setw(10) << total_rle2_time << " ms\n";
    std::cout << "  5. Huffman Encoder:    " << std::setw(10) << total_huff_time << " ms\n";
    std::cout << "======================================================\n";
    std::cout << "💾 PIPELINE OUTPUT COMMITTED TO DISK SEAMLESSLY.\n";
    std::cout << "======================================================\n\n";
